...
//  TODO: Turn this into a npm package
import { FC, useRef, useEffect, useState } from "react";
import styled from "styled-components";

interface BubbleConfig {
	compose: string; // default is "lighter"  TODO: Type this
	animate: boolean; // default is true
	bubbles: number; // default is Math.random() * 100
	shadowColor: string; // default is #fff
	shadowBlur: number; // default is 4
	strokeColor: string; // default is 'rgba(255, 255, 255, 0.1)'
	strokeWidth: number; // default is Math.random() * 5
	fill: string; // default is `hsla(0, 0%, 100%, ${Math.random() * 0.02})`'
	angle: number; // default is Math.random() * Math.PI * 2
	velocity: number; // default is 0.1 + Math.random() * 0.1
	radius: number; // 4 + Math.random() * window.innerWidth / 8
}
export const Bubbles: FC<BubbleConfig> = ({
	compose = "lighter",
	animate = true,
	bubbles = 5,
	shadowColor = "#fff",
	...
	radius = 4 + (Math.random() * window.innerWidth) / 8,
}) => {
...
// TODO: Continue making the bubbles component
